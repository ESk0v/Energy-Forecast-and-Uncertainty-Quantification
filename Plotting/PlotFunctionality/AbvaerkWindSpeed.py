...
# === SCATTER PLOT: Wind Speed vs Abvaerk ===
correlation = np.corrcoef(Data['abvaerk'], Data['windSpeed_0'])[0, 1]

fig_scatter = plt.figure(figsize=(10, 7))
plt.scatter(Data['windSpeed_0'], Data['abvaerk'], alpha=0.3, s=10)
plt.xlabel('Wind Speed (m/s)')
plt.ylabel('Energy Production (MWh)')
plt.title(f'Wind Speed vs Energy Production (r = {correlation:.3f})')
plt.grid(True, alpha=0.3)
plt.tight_layout()
SavePlot(fig_scatter, "Abvaerk_WindSpeed_Correlation.png")
...
# Windspeed on secondary axis
ax2 = ax1.twinx()
ax2.plot(weekly['week_str'], weekly['windSpeed_0'], color='blue', linewidth=2, marker='s', markersize=3, label='Avg Wind Speed (m/s)')
ax2.set_ylabel('Avg Wind Speed (m/s)', color='blue')
ax2.tick_params(axis='y', labelcolor='blue')

weekly_corr = np.corrcoef(weekly['abvaerk'], weekly['windSpeed_0'])[0, 1]
plt.title(f'Weekly Average: Energy Production vs Wind Speed (r = {weekly_corr:.3f})')
fig_weekly.tight_layout()
SavePlot(fig_weekly, "Abvaerk_WindSpeed_WeeklyAverage.png")
