...
def get_data(filepath=CSV_FILE_PATH):
    ...
    # Convert comma decimals for precipitation_0 (the one we need for plotting)
    df['precipitation_0'] = pd.to_numeric(df['precipitation_0'].astype(str).str.replace(',', '.'), errors='coerce')

    # Drop rows with NaN values in the columns we need
    df = df.dropna(subset=['abvaerk', 'precipitation_0', 'dateTime'])
...
def plot_correlation(df):
    """Scatter plot showing correlation between precipitation and energy usage."""
    correlation = np.corrcoef(df['abvaerk'], df['precipitation_0'])[0, 1]

    fig = plt.figure(figsize=(10, 7))
    plt.scatter(df['precipitation_0'], df['abvaerk'], alpha=0.3, s=10)
    plt.xlabel('Precipitation (mm)')
    plt.ylabel('Energy Usage (MWh)')
    plt.title(f'Precipitation vs Energy Usage (r = {correlation:.3f})')
...
def plot_weekly_average(df):
    ...
    correlation = np.corrcoef(weekly['abvaerk'], weekly['precipitation_0'])[0, 1]

    plt.title(f'Weekly Average: Energy Usage vs Precipitation (r = {correlation:.3f})')
    fig.tight_layout()
    # plt.show()
...
if __name__ == "__main__":
    ...
    # Save the plot
    SavePlot(fig1, "Abvaerk_Precipitation_WeeklyAverage.png")
    SavePlot(fig2, "Abvaerk_Precipitation_Correlation.png", "Images/Correlation")